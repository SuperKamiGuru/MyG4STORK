...
//
#include "../include/STORKNeutronHPEnAngCorrelation.hh"
#include "G4LorentzRotation.hh"
#include "G4LorentzVector.hh"
#include "G4RotationMatrix.hh"

G4ReactionProduct * STORKNeutronHPEnAngCorrelation::SampleOne(G4double anEnergy)
{
...
G4ReactionProductVector * STORKNeutronHPEnAngCorrelation::Sample(G4double anEnergy)
{
  ...
  G4LorentzRotation toLab(toZ.inverse()); //toLab only change axis NOT to LAB system
    ...
    {
      G4double sum=0;
      G4double * running = new G4double[nProducts];
      running[0] = 0;

      for (i=0; i<nProducts; i++)
      {
        if (i!=0) running[i]=running[i-1];
        running[i]+=theProducts[i].GetFractionalProbability(anEnergy);
      }
      sum = running[nProducts-1];
    ...
    if(it!=0)
    {
      for(unsigned int ii=0; ii<it->size(); ii++)
      {
        ...
        {
    #ifdef G4_NHP_DEBUG
              cout <<"STORKNeutronHPEnAngCorrelation: "<<
        ...
        {
              throw G4HadronicException(__FILE__, __LINE__, "STORKNeutronHPEnAngCorrelation::Sample: The frame of the finalstate is not specified");
        }
