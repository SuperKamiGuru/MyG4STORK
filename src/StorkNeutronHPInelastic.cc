...
#include "G4NeutronHPManager.hh"
  ...
  StorkNeutronHPInelastic::StorkNeutronHPInelastic(G4double finalStateTemp)
    :G4HadronicInteraction("NeutronHPInelastic")
  {
    ...
    if ( istatus < 0 )
    {
      G4cout << "Warning! system(\"echo $G4NEUTRONHPDATA\") returns error value at StorkNeutronHPInelastic" << G4endl;
    }

//    G4cout << " entering StorkNeutronHPInelastic constructor"<<G4endl;
...
/*
    theInelastic = new StorkNeutronHPChannelList[numEle];
...
*/
    ...
    for (G4int i=0; i<numEle; i++)
    {
      theInelastic.push_back( new StorkNeutronHPChannelList(fsTemp) );
      (*theInelastic[i]).Init((*(G4Element::GetElementTable()))[i], dirName);
      G4int itry = 0;
      do
      {
  ...
  StorkNeutronHPInelastic::~StorkNeutronHPInelastic()
  {
//    delete [] theInelastic;
     for ( std::vector<StorkNeutronHPChannelList*>::iterator
  ...
  G4HadFinalState * StorkNeutronHPInelastic::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& aNucleus )
  {
    ...
    if(n!=1)
    {
      ...
      for (i=0; i<n; i++)
      {
        ...
        xSec[i] *= rWeight;
//        if((*theInelastic[index]).GetXsec(aTrack.GetKineticEnergy())!=0.)
//        {
//            xSec[i] = rWeight;
//        }
//        else
...
const std::pair<G4double, G4double> StorkNeutronHPInelastic::GetFatalEnergyCheckLevels() const
{
...
void StorkNeutronHPInelastic::addChannelForNewElement()
{
   for ( G4int i = numEle ; i < (G4int)G4Element::GetNumberOfElements() ; i++ )
   {
      G4cout << "StorkNeutronHPInelastic Prepairing Data for the new element of " << (*(G4Element::GetElementTable()))[i]->GetName() << G4endl;
...
G4int StorkNeutronHPInelastic::GetVerboseLevel() const
{
   return G4NeutronHPManager::GetInstance()->GetVerboseLevel();
}
void StorkNeutronHPInelastic::SetVerboseLevel( G4int newValue )
{
