...
//
#include "../include/G4NeutronHPAngularEnergy3DTab.hh"
...
void G4NeutronHPAngularEnergy3DTab::Init(std::istream & aDataFile)
{
  aDataFile >> nEnergies;
  theManager.Init(aDataFile);
  theEnergies = new G4double[nEnergies];
  nEOut = new G4int[nEnergies];
  nEProbSum = new G4double *[nEnergies];
  ...
  for(G4int i=0; i<nEnergies; i++)
  {
    ...
    theEnergies[i]*=eV;
    aDataFile >> nEOut[i];
    nEProbSum[i] = new G4double[nEOut[i]];
    theSecondManager[i].Init(aDataFile);
    theData[i] = new G4NeutronHPVector[nEOut[i]];
    G4double label;
    for(G4int ii=0; ii<nEOut[i]; ii++)
    {
      aDataFile >> label;
      aDataFile >> nEProbSum[i][ii];
      theData[i][ii].SetLabel(label*eV);
      theData[i][ii].Init(aDataFile);
    }
  }
}

G4ReactionProduct * G4NeutronHPAngularEnergy3DTab::Sample(G4double anEnergy, G4double massCode, G4double )
{
   ...
   {
     throw G4HadronicException(__FILE__, __LINE__, "G4NeutronHPAngularEnergy3DTab: Unknown ion case 2");
   }
   ...
   {
     // integrate the prob for each eOut, and select theta.
     G4double random = nEProbSum[it][nEOut[it]-1]*G4UniformRand();
     G4int ith(0);
     for(i=0;i<nEOut[it]; i++)
     {
       ith = i;
       if(random<nEProbSum[it][i]) break;
     }
     //080807
     //if ( ith == 0 || ith == nEOut[it]-1 ) //ith marks the angluar bin
     if ( ith == 0 ) //ith marks the angluar bin
     {
        secEnergy = theData[it][ith].GetLabel();
        cosTh = theData[it][ith].Sample();
     }
     else
     {
       //080808
       ...
       G4double x1 = nEProbSum[it][ ith-1 ];
       G4double x2 = nEProbSum[it][ ith ];
       G4double x = random;
       G4double y1 = theData[it][ith-1].GetLabel();
       G4double y2 = theData[it][ith].GetLabel();
       secEnergy = theInt.Interpolate(theSecondManager[it].GetScheme(ith),
                                  x, x1, x2, y1, y2);
   ...
   {
       G4double cosThTemp[2], secEnergyTemp[2]/*, rand=G4UniformRand()*/;

//       for(G4int inEIdx=-1; inEIdx<1; inEIdx++)
//       {
//            // integrate the prob for each costh, and select theta.
//            G4double random = nEProbSum[it+inEIdx][nEOut[it+inEIdx]-1]*rand;
////            G4int ith(0);
////            for(i=0;i<nEOut[it+inEIdx]; i++)
////            {
////                ith = i;
////                if(random<nEProbSum[it+inEIdx][i]) break;
////            }
////            if ( ith == 0 ) //ith marks the angluar bin
////            {
////                secEnergyTemp[inEIdx+1] = theData[it+inEIdx][ith].GetLabel();
////            }
////            else
////            {
////                G4double x1 = nEProbSum[it+inEIdx] [ ith-1 ];
////                G4double x2 = nEProbSum[it+inEIdx] [ ith ];
////                G4double x = random;
////                G4double y1 = theData[it+inEIdx][ith-1].GetLabel();
////                G4double y2 = theData[it+inEIdx][ith].GetLabel();
////                secEnergyTemp[inEIdx+1] = theInt.Interpolate(theSecondManager[it+inEIdx].GetScheme(ith),
////                                          x, x1, x2, y1, y2);
////            }
//
//            G4double x1 = nEProbSum[it+inEIdx] [ 0 ];
//            G4double x2 = nEProbSum[it+inEIdx] [ nEOut[it+inEIdx]-1 ];
//            G4double x = random;
//            G4double y1 = theData[it+inEIdx][0].GetLabel();
//            G4double y2 = theData[it+inEIdx][nEOut[it+inEIdx]-1].GetLabel();
//            secEnergyTemp[inEIdx+1] = theInt.Interpolate(theSecondManager[it+inEIdx].GetScheme(0),
//                                      x, x1, x2, y1, y2);
//       }
//
//        secEnergy = theInt.Lin(anEnergy,theEnergies[it-1],theEnergies[it],secEnergyTemp[0],secEnergyTemp[1]);

        {
            G4int inEIdx=-1, low[2]={0,0}, high[2]={nEOut[it-1]-1,nEOut[it]-1};
            if(((anEnergy-theEnergies[it-1])/(theEnergies[it]-theEnergies[it-1]))>G4UniformRand())
            {
                inEIdx=0;
            }
            for(i=0;i<2;i++)
            {
                for(G4int j=0;j<nEOut[it-1+i]-1;j++)
                {
                    if(nEProbSum[it-1+i][j+1]>1.e-20)
                    {
                      low[i]=j;
                      break;
                    }
            ...
            for(i=0;i<2;i++)
            {
                for(G4int j=nEOut[it-1+i]-1;j>0;j--)
                {
                    if(nEProbSum[it-1+i][j-1]!=nEProbSum[it-1+i][j])
                    {
                      high[i]=j;
                      break;
                    }
            ...
            secEnergyTemp[0] = theInt.Lin(anEnergy,theEnergies[it-1],theEnergies[it],theData[it-1][low[0]].GetLabel(),theData[it][low[1]].GetLabel());
            secEnergyTemp[1] = theInt.Lin(anEnergy,theEnergies[it-1],theEnergies[it],theData[it-1][high[0]].GetLabel(),theData[it][high[1]].GetLabel());

            G4int ith(0);
            G4double random = nEProbSum[it+inEIdx][nEOut[it+inEIdx]-1]*G4UniformRand();
            for(i=0;i<nEOut[it+inEIdx]; i++)
            {
                ith = i;
                if(random<nEProbSum[it+inEIdx][i]) break;
            ...
            if ( ith == 0 ) //ith marks the angluar bin
            {
                secEnergy = theData[it+inEIdx][0].GetLabel();
            }
            else
            {
                secEnergy = theInt.Lin(random,nEProbSum[it+inEIdx][ith-1],nEProbSum[it+inEIdx][ith],theData[it+inEIdx][ith-1].GetLabel(),theData[it+inEIdx][ith].GetLabel());
            }

            secEnergy = theInt.Lin(secEnergy,theData[it+inEIdx][low[inEIdx+1]].GetLabel(),theData[it+inEIdx][high[inEIdx+1]].GetLabel(),secEnergyTemp[0],secEnergyTemp[1]);
        }
       ...
       for(G4int inEIdx=-1; inEIdx<1; inEIdx++)
       {
            ...
            {
                ith = i;
                if(secEnergy<theData[it+inEIdx][ith].GetLabel()) break;
            }
            G4double x1 = nEProbSum[it+inEIdx] [ ith-1 ];
            G4double x2 = nEProbSum[it+inEIdx] [ ith ];
            G4double y1 = theData[it+inEIdx][ith-1].GetLabel();
            ...
            if ( ith == 0 ) //ith marks the angluar bin
            {
                cosThTemp[inEIdx+1] = theData[it][ith].Sample();
            }
            else
            {
                G4NeutronHPVector theBuff1;
                theBuff1.SetInterpolationManager(theData[it+inEIdx][ith-1].GetInterpolationManager());
                G4NeutronHPVector theBuff2;
                theBuff2.SetInterpolationManager(theData[it+inEIdx][ith].GetInterpolationManager());
                x1=y1;
                x2=y2;
                G4double y, mu;
                for(i=0;i<theData[it+inEIdx][ith-1].GetVectorLength(); i++)
                {
                     mu = theData[it+inEIdx][ith-1].GetX(i);
                     y1 = theData[it+inEIdx][ith-1].GetY(i);
                     y2 = theData[it+inEIdx][ith].GetY(mu);

                     y = theInt.Interpolate(theSecondManager[it+inEIdx].GetScheme(ith),
                                            secEnergy, x1,x2,y1,y2);
                     theBuff1.SetData(i, mu, y);
                }
                for(i=0;i<theData[it+inEIdx][ith].GetVectorLength(); i++)
                {
                     mu = theData[it+inEIdx][ith].GetX(i);
                     y1 = theData[it+inEIdx][ith-1].GetY(mu);
                     y2 = theData[it+inEIdx][ith].GetY(i);
                     y = theInt.Interpolate(theSecondManager[it+inEIdx].GetScheme(ith),
                                            secEnergy, x1,x2,y1,y2);
                     theBuff2.SetData(i, mu, y);
                }
                G4NeutronHPVector theStore;
                theStore.Merge(&theBuff1, &theBuff2);
                cosThTemp[inEIdx+1] = theStore.Sample();
            }
       }
       cosTh = theInt.Lin(anEnergy,theEnergies[it-1],theEnergies[it],cosThTemp[0],cosThTemp[1]);
