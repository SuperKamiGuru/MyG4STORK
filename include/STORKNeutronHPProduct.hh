...
//
//
#ifndef STORKNeutronHPProduct_h
#define STORKNeutronHPProduct_h 1
...
#include "G4NeutronHPAngularEnergy3DTab.hh"

class STORKNeutronHPProduct
{
  public:
  STORKNeutronHPProduct()
  {
    theDist = 0;
  }
  ~STORKNeutronHPProduct()
  {
    if(theDist != 0) delete theDist;
      ...
      //STORKNeutronHPEnAngCorrelation
  ...
  inline void Init(std::istream & aDataFile)
  {
    ...
    else if(theDistLaw == 66)
    {
        theDist = new G4NeutronHPNBodyPhaseSpaceMCNP;
    }
    ...
    else if(theDistLaw == 61)
    {
      // Laboratory angular energy paraetrisation
      theDist = new G4NeutronHPAngularEnergy3DTab;
    }
    else
    {
      throw G4HadronicException(__FILE__, __LINE__, "distribution law unknown to STORKNeutronHPProduct");
    }
    if(theDist!=0)
    {
